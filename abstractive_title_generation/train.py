...
with open('vocabulary-embedding/vocabulary-embedding.data.pkl', 'rb') as fp:
	X, Y = pickle.load(fp)
...
def lpadd(x, maxlend=maxlend, eos=eos):
	# left (pre) pad a description to maxlend and then add eos. The eos is the input to predicting the first word in the headline
	assert maxlend >= 0
	if maxlend == 0:
		return [eos]
	n = len(x)
...
def conv_seq_labels(xds, xhs, nflips=None, model=None, debug=False):
	...
	for i, xh in enumerate(xhs):
		...
		xh = xh[:maxlenh]
		y[i,:,:] = to_categorical(xh, vocab_size)
	return x, y
...
for iteration in range(1):
	...
	with open('./models/train.history.pkl','wb') as fp:
		pickle.dump(history,fp,-1)
	model.save_weights('./models/train.hdf5', overwrite=True)
	# gensamples(batch_size=batch_size)

# model.save('./models/model_headliner.h5')